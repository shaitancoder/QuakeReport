...
public final class QueryUtils {
    ...
    public static ArrayList<Flavour> extractEarthquakes()   {

        ArrayList<Flavour> earthquakes = new ArrayList<>();

        JSONObject reader = null;
        try {
            reader = new JSONObject(SAMPLE_JSON_RESPONSE);
        } catch (JSONException e) {
            e.printStackTrace();
        }
        JSONArray earthquakearray = null;
        try {
            earthquakearray = reader.getJSONArray("features");
        } catch (JSONException e) {
            e.printStackTrace();
        }
        ...
        try {
            ...
            for (int i = 0; i < earthquakearray.length(); i++) {
                ...
                Flavour earthquake = new Flavour(magnitude, place, time,url);
